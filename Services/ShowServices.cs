...
namespace BookMyShow.Services
{
    public class ShowServices:IShowServices
    {
        ...
        public List<Movie> GetByMovieAndCity(string MovieName, string City)
        {
            ...
            var sqlQuery = "SELECT distinct m.Id,m.Name,m.ImbdRating,m.Category,m.Genre,m.ReleaseDate,m.ActorName,m.ActressName FROM ( Show s join Theatre t on s.TheatreId = t.Id ) join Movie m on m.Id = s.MovieId where ( @MovieName is null or m.Name like '%' + @MovieName + '%' ) and ( @City is null or t.City = @City ) ";
        ...
        public Show GetByMovieAndTheatre(int MovieId, int TheatreId)
        {
            ...
            return _db.Query<Show>(sqlQuery, new { @MovieId = MovieId, @TheatreId = TheatreId }).FirstOrDefault();
        }

        public List<Show> GetByMovieId(int MovieId)
        {
            var sqlQuery = "SELECT * FROM Show where MovieId = @MovieId";
            return _db.Query<Show>(sqlQuery, new { @MovieId = MovieId }).ToList();
        }

        public List<Show> GetByTheatreId(int TheatreId)
        {
            var sqlQuery = "SELECT * FROM Show where TheatreId = @TheatreId";
            return _db.Query<Show>(sqlQuery, new { @TheatreId = TheatreId }).ToList();
        }
    }
}
